...
// If user is already logged in, redirect to FreedomNarratives page
...
if (!empty($_POST['email']) && !empty($_POST['password'])):
  ...
  // Fix: Ensure $results is not false to avoid the count() issue
  if ($results && $_POST['password'] == $results['password'] && $results['security'] != "R") {

    $_SESSION['user_id'] = $results['id'];

    // Check if the user already exists in the login_dump table
    $sql = "SELECT COUNT(*) as count FROM `login_dump` WHERE Username = :email";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':email', $_POST['email']);
    $stmt->execute();
    $loginDumpResult = $stmt->fetch(PDO::FETCH_ASSOC);

    // Insert the login details only if the user does not exist in login_dump
    if ($loginDumpResult['count'] == 0) {
      $sql = "INSERT INTO `login_dump` (`Username`, `TimeDate`) VALUES (:email, :TimeDate)";
      $stmt = $conn->prepare($sql);
      $stmt->bindParam(':email', $_POST['email']);
    ...
    // Redirect to FreedomNarratives after login
    header("Location: FreedomNarratives");

  } else {
    // If the user is restricted
    if (isset($results['security']) && $results['security'] == "R") {
              ...
              <div class="card" style="width: 100%; opacity: 0.7; margin-top: 3%; ">
                <div class="card-body">
                  ...
                  <p class="card-text">For free sign-up and access to this portal, please email us your details at:
                    <a href="mailto:support@regid.ca" class="card-link">support@regid.ca</a>
                  </p>
    ...
    <article class="panel__wrapper">
      <div class="panel__content">
        ...
        <p><i>Decoding Origins Web Portal</i> addresses the concerns of accessible
          ...
          analysis of this textual and visual information by scholars of historical and contemporary of Africa. We have
          developed
    ...
    <article class="panel__wrapper">
      <div class="panel__content">
        ...
        <p>The <i>Decoding Origin Web Portal</i> is developed as part of a collaboration between
          ...
          <a href="http://ocadu.ca">OCAD University</a> and <a href="https://www.yorku.ca/index.html">York
            University</a>.
          ...
          <span style="font-size:14px;">
            ...
            <i>Kartikay Chadha, Katrina Keefer and Martha Ladly, <i>“Decoding Origins Web Portal: Creating a
              ...
              Walter Hawthorne, and Dean Rehberger, editors. Encoding <i><a
                  href="http://enslaved.org">Enslaved.org</a></i>:
